...
function adicionarProduto(nome, preco, quantidade) {
    ...
    produtos.set(novoProduto.id, novoProduto);
    obterId++;
...
function venderProduto(id, quantidade) {
    const produtoVendido = produtos.get(id);
    ...
    if (produtoVendido) {
        ...
        if (produtoVendido.quantidade >= quantidade) {
            ...
            return `Você comprou ${quantidade} unidades de ${produtoVendido.nome} (ID: ${id}) por R$${valorTotal.toFixed(2)}`;
        } else {
...
function gerarBoletoVendas() {
    ...
    boleto += `Total: R$${calcularTotalVendas()}\n`;
    boleto += "=============================";
    return boleto;
}
...
function atualizarProdutoMenu() {
    ...
    while (continuarAtualizar) {
        const idAtualizar = parseInt(prompt("Digite o ID do produto a ser atualizado (ou 0 para voltar ao menu):"));
        ...
        if (isNaN(novoPreco) || isNaN(novaQuantidade)) {
            ...
            console.log(atualizarProduto(idAtualizar, novoNome, novoPreco, novaQuantidade));
        }
    }
}
...
function removerProdutoMenu() {
    ...
    while (continuarRemover) {
        const idRemover = parseInt(prompt("Digite o ID do produto a ser removido (ou 0 para voltar ao menu):"));
...
function venderProdutoMenu() {
    ...
    while (continuarVendas) {
        const idVender = parseInt(prompt("Digite o ID do produto a ser vendido (ou 0 para voltar ao menu):"));
...
function relatorioEstoque(tipoRelatorio) {
    ...
    switch (tipoRelatorio) {
        case 1:
            produtosRelatorio = Array.from(produtos.values());
            break;
        case 2:
            produtosRelatorio = Array.from(produtos.values()).filter((produto) => produto.quantidade > 0);
            break;
        case 3:
            produtosRelatorio = Array.from(produtos.values()).filter((produto) => produto.quantidade === 0);
            break;
    ...
    const relatorio = produtosRelatorio.reduce((acc, produto) => {
        acc.push(`ID: ${produto.id}, Nome: ${produto.nome}, Preço: R$${produto.preco.toFixed(2)}, Quantidade: ${produto.quantidade}`);
        return acc;
    }, []);
...
function exibirRelatorio(tipoRelatorio) {
    switch (tipoRelatorio) {
        case "1":
            ...
            console.log(relatorioEstoque(1).join("\n"));
            break;
        case "2":
            console.clear();
            console.log("Relatório: Produtos em Estoque");
            console.log(relatorioEstoque(2).join("\n"));
            break;
        case "3":
            console.clear();
            console.log("Relatório: Produtos sem Estoque");
            console.log(relatorioEstoque(3).join("\n"));
            break;
        case "0":
            console.clear();
            exibirMenu();
